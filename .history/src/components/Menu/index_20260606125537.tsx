...
/* Leisurely panel slide + scrim blur (see GSAP in component). Nav picks
   up the same open duration so the cascade stays in sync. */
const MENU_PANEL_OPEN_DURATION = 1.44;
const MENU_PANEL_CLOSE_DURATION = 1.06;
/** Scrim dim alpha (Modal-style); driven via `--menu-scrim-a` on `.backdrop`. */
const MENU_SCRIM_ALPHA_OPEN = 0.48;
/** Modal / header-adjacent ease — calm, not snappy. */
...
/** When amp→0 the clip is the full panel (y=1). Wave only bites upward from the bottom. */
const MENU_WAVE_AMP_START = 0.078;
/** ~one gentle hump across the width. */
const MENU_WAVE_FREQ = 1.08;
const MENU_WAVE_PATH_STEPS = 48;
/** Wave settles a touch after the panel lands — reads calmer. */
const MENU_WAVE_SETTLE_DURATION = MENU_PANEL_OPEN_DURATION * 1.2;

/** `clipPathUnits="objectBoundingBox"` path for the menu panel. */
function buildMenuWaveClipPath(amp: number): string {
  if (amp < 0.001) {
    return "M 0 0 L 1 0 L 1 1 L 0 1 Z";
  }
  /* Bottom edge: y = 1 at sin min (full height), y = 1−amp at sin max (deepest bite).
     Fixes the old symmetric wave around 0.9, which hid ~10% of the panel (~100px). */
  const yAt = (x: number) =>
...
export function Menu({
  ...
  /* Panel slides from top; scrim blurs/dims the page behind (GSAP, Modal-style). */
  useEffect(() => {
    ...
    /* Closed — first mount: snap up without animating (avoids a flash). */
  ...
  /* Lock viewport scroll (both html and body) while the menu is open.
     Lenis listens to wheel/touch on the window — explicit `stop()` is
     the cleanest way to make sure no momentum carries on under the
     overlay even on edge cases (touch devices, trackpad inertia). */
